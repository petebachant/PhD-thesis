...
simulations are available from XXX and XXX. \todo[inline]{Add citation for case
    files here. Should all be in one Figshare repo, or 4 different repos.}
...
a single processor and took on the order of one CPU hour per second. Mean
performance was calculated from \todo[inline]{Describe how mean performance was
    calculated, i.e., intervals.}

Flow statistics were calculated from a time series starting from $t=xxx$ to
$f=xxx$ \todo[inline]{Describe wake statistic calculations.} and downsampled to
50 Hz. It is assumed that this is far enough from the blade passage frequency
that differences from the variance in the original velocity will be negligible.
...
with the Spalart--Allmaras model performing better than the SST model. The
apparent overprediction of rotor drag coefficient could be an effect of the
experimental procedure, where the ``tare drag'' from the turbine mounting
structure was measured without a turbine installed, then subtracted in
