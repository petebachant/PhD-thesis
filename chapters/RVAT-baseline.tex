...
\nomenclature{LDV}{Laser-Doppler velocimetry.}
...
\nomenclature{$Re_D$}{Reynolds number based on turbine rotor diameter.}
...
\nomenclature{$H$}{Turbine rotor height.}
...
from Figures 6 and 4. Note that despite lower turbulence kinetic energy on the
$+y$ side of the turbine, magnitudes of the turbulent transport are similar on
both.
...
Like the streamwise momentum transport, contributions to mean kinetic energy
...
OpenFOAM's \texttt{actuationDiskSource} are given in Table~\ref{tab:AS}, and the
case files for this simulation are available from
...
\nomenclature{$D$}{Turbine rotor diameter.}
...
\begin{figure}
    ...
    width=0.8\textwidth]{AD_meancontquiv}
...
\begin{figure}
    ...
    \includegraphics[width=0.87\textwidth]{AD_streamwise}
