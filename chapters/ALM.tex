...
It was determined that the Goude model performed best with respect to near-wake
predictions, while performance predictions from both models were similar. Thus,
the Goude model was used for all simulations reported here. \todo[inline]{Check
    that Mandal--Burton perf predictions are similar.}
...
efficiency. \todo[inline]{Cite turbinesFoam code repository here.}
...
conditions. Pressure--velocity coupling for the momentum equation was achieved
...
\todo[inline]{Add slice of the ALM mesh.}
\begin{figure}
    \centering
...
RM2 cases. Time steps were chosen as $\Deltat = 0.01$ and $\Delta t = 0.005$
...
computational domain was extended slightly to allow for further wake
development.
...
\todo[inline]{Mention LES performance.}
...
\todo[inline]{Verify calculation of LES turbulent transport. May only use SGS.}
...
An actuator line model for cross-flow turbines was developed and validated
against experimental datasets acquired for high and medium to low solidity
rotors at scales where the performance and near-wake dynamics were essentially
Reynolds number independent. When coupled to a $k$--$\epsilon$ RANS solver ALM
...
that of max power coefficient. The RM2 power coefficient on the other hand was
underpredicted at lower $\lambda$. Both models overestimate $C_P$ at the highest
tip speed ratios, which has been observed in other simulations using
Leishman--Beddoes type dynamic stall models. Possible explanations include
underestimation of added mass effects or blade--strut connection corner drag,
incorrect time constants in the LB DS model, and/or inaccuracy due to the
virtual camber effect. In the present flow curvature model, the angle of attack
is corrected, but the foil coefficient data is not transformed, meaning the LB
DS separation point curve fit parameters are equal for both positive and
negative angles of attack. A foil data transformation algorithm based on virtual
camber should be investigated for future improvement of the ALM.
...
high-fidelity flow modeling, and will prove very useful. The prospect of using
ALM simulations to drive down computational cost of RANS by nearly five orders
of magnitude justifies further development.
...
The ALM should be evaluated for its use in modeling CFT arrays using both RANS
