...
The actuator line model developed here, dubbed \textit{turbinesFoam}, was
written as an extension library for the OpenFOAM finite volume CFD library.
OpenFOAM is a free and open-source, and is widely used throughout industry and
academia. \textit{turbinesFoam} was also developed freely and openly from the
start in order to increase community engagement and research efficiency by
allowing other researchers to incrementally improve the model, rather than
starting from nothing.

The code was written in OpenFOAM's style, similar to NREL's SOWFA, but uses
OpenFOAM's \texttt{fvOptions} framework for adding source terms to equations at
...
Furthermore, unlike SOWFA, custom solvers do not need to be developed and
maintained to include the CFT-ALM.
...
down to the element level, which can be confusing to read. Furthermore, adapting
\textit{SOWFA} for CFTs would nearly double the amount of code to debug and
maintain.
