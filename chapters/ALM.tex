...
rotors at large laboratory scale. When coupled to a $k$--$\epsilon$ RANS solver
simulations took $O(0.1)$ CPU hours per second of simulated time, while when
coupled to a Smagorinsky LES model took $O(10)$, which represent a four and two
order of magnitude decrease in computational expense versus blade-resolved RANS,
respectively.
...
high-fidelity flow modeling, and will prove very useful.
