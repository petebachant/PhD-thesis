...
Blade element based potential flow vortex line methods, e.g.,
\cite{Strickland1981,Murray2011}, can be solved with reasonable computational
effort---ranging from an expense close to BEM up to that of 2-D blade-resolved
...
CFD library. Note that the content here has been submitted for publication
as~\cite{Bachant2016-VAT-ALM}.
...
local fluid velocity. The blade element lift force, drag force, and pitching
...
\nomenclature{$F_l$}{Blade element lift force.}
\nomenclature{$F_d$}{Blade element drag force.}
\nomenclature{$M$}{Blade element pitching moment.}
...
\nomenclature{ALE}{Actuator line element.}
...
turbine using LES \cite{Shamsoddin2014}. The US National Renewable Energy Lab
(NREL) SOWFA ALM for OpenFOAM also uses the velocity at the actuator line
element location for computing inflow velocity and local angle of attack with no
corrections \cite{Churchfield2013}.
...
\nomenclature{$\epsilon$}{Actuator line element force Gaussian smearing width.}
...
This is indicative of inaccurate dynamic stall modeling, which could be
attributed to one of the models' tuning constants, e.g., the time constant
$T_\alpha$ for the lagged angle of attack $\alpha^\prime$, the change of which
per time step is given by~\cite{Sheng2008}
\begin{equation}
    \Delta \alpha^\prime = \Delta \alpha (s) (1 - \mathrm{e}^{-s/T_\alpha}),
    \label{eq:alpha-prime}
\end{equation}
where $s$ is a nondimensional time, defined as~\cite{Leishman1989}
\begin{equation}
    s = \frac{2 | \vec{U}_{\mathrm{rel}} | t}{c}.
\end{equation}

Limited ad hoc testing revealed that the mean $C_P$ at $\lambda_0$ more closely
matched experimental measurements with $T_\alpha$ roughly double the default
value given in~\cite{Sheng2008}. This could be investigated further by looking
at the phase of, e.g., maximum $C_P$ peaks compared to the experimental values
(or possibly those from blade-resolved CFD), but for the present study only mean
